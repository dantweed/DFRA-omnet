...
#include "inet/linklayer/ieee80211/mac/FrameExchanges.h"
...
void DfraEdcaUpperMac::initialize()
{
...
IMacParameters *DfraEdcaUpperMac::extractParameters(const IIeee80211Mode *slowestMandatoryMode)
{
...
void DfraEdcaUpperMac::handleMessage(cMessage *msg)
{
...
void DfraEdcaUpperMac::upperFrameReceived(Ieee80211DataOrMgmtFrame *frame)
{
...
void DfraEdcaUpperMac::enqueue(Ieee80211DataOrMgmtFrame *frame, AccessCategory ac)
{
...
AccessCategory DfraEdcaUpperMac::classifyFrame(Ieee80211DataOrMgmtFrame *frame)
{
...
AccessCategory DfraEdcaUpperMac::mapTidToAc(int tid)
{
...
void DfraEdcaUpperMac::lowerFrameReceived(Ieee80211Frame *frame)
{
...
void DfraEdcaUpperMac::corruptedFrameReceived()
{
    int numACs = params->isEdcaEnabled() ? 4 : 1;
    for (int i = 0; i < numACs; i++)
        if (acData[i].frameExchange)
...
void DfraEdcaUpperMac::channelAccessGranted(IContentionCallback *callback, int txIndex)
{
    Enter_Method("channelAccessGranted()");
    callback->channelAccessGranted(txIndex);
}

void DfraEdcaUpperMac::internalCollision(IContentionCallback *callback, int txIndex)
{
...
void DfraEdcaUpperMac::transmissionComplete(ITxCallback *callback)
{
...
void DfraEdcaUpperMac::startSendDataFrameExchange(Ieee80211DataOrMgmtFrame *frame, int txIndex, AccessCategory ac)
{
...
void DfraEdcaUpperMac::frameExchangeFinished(IFrameExchange *what, bool successful)
{
...
void DfraEdcaUpperMac::sendAck(Ieee80211DataOrMgmtFrame *frame)
{
    Ieee80211ACKFrame *ackFrame = utils->buildAckFrame(frame);
    tx->transmitFrame(ackFrame, params->getSifsTime(), nullptr);
}

void DfraEdcaUpperMac::sendCts(Ieee80211RTSFrame *frame)
{
